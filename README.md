...
To see a full example, check out the [main.tf](https://github.com/sourcefuse/terraform-aws-arc-document-db/blob/main/examples) file in the example folder.  
...
|------|-------------|------|---------|:--------:|
| <a name="input_alarm_actions"></a> [alarm\_actions](#input\_alarm\_actions) | The list of actions to execute when this alarm transitions into an ALARM state | `list(string)` | `[]` | no |
...
| <a name="input_allowed_cidr_blocks"></a> [allowed\_cidr\_blocks](#input\_allowed\_cidr\_blocks) | List of CIDR blocks allowed to access the DocumentDB cluster (legacy support) | `list(string)` | `[]` | no |
| <a name="input_allowed_security_group_ids"></a> [allowed\_security\_group\_ids](#input\_allowed\_security\_group\_ids) | List of security group IDs allowed to access the DocumentDB cluster (legacy support) | `list(string)` | `[]` | no |
...
| <a name="input_connection_alarm_evaluation_periods"></a> [connection\_alarm\_evaluation\_periods](#input\_connection\_alarm\_evaluation\_periods) | The number of periods over which data is compared to the specified threshold for connection alarm | `number` | `2` | no |
| <a name="input_connection_alarm_period"></a> [connection\_alarm\_period](#input\_connection\_alarm\_period) | The period in seconds over which the specified statistic is applied for connection alarm | `number` | `300` | no |
| <a name="input_connection_alarm_threshold"></a> [connection\_alarm\_threshold](#input\_connection\_alarm\_threshold) | The value against which the specified statistic is compared for database connections | `number` | `80` | no |
...
| <a name="input_cpu_alarm_evaluation_periods"></a> [cpu\_alarm\_evaluation\_periods](#input\_cpu\_alarm\_evaluation\_periods) | The number of periods over which data is compared to the specified threshold for CPU alarm | `number` | `2` | no |
| <a name="input_cpu_alarm_period"></a> [cpu\_alarm\_period](#input\_cpu\_alarm\_period) | The period in seconds over which the specified statistic is applied for CPU alarm | `number` | `300` | no |
| <a name="input_cpu_alarm_threshold"></a> [cpu\_alarm\_threshold](#input\_cpu\_alarm\_threshold) | The value against which the specified statistic is compared for CPU utilization | `number` | `80` | no |
| <a name="input_create_cloudwatch_alarms"></a> [create\_cloudwatch\_alarms](#input\_create\_cloudwatch\_alarms) | Whether to create CloudWatch alarms for monitoring | `bool` | `false` | no |
| <a name="input_create_db_cluster_parameter_group"></a> [create\_db\_cluster\_parameter\_group](#input\_create\_db\_cluster\_parameter\_group) | Whether to create a DB cluster parameter group | `bool` | `false` | no |
...
| <a name="input_create_event_subscription"></a> [create\_event\_subscription](#input\_create\_event\_subscription) | Whether to create a DocumentDB event subscription | `bool` | `false` | no |
...
| <a name="input_create_secret"></a> [create\_secret](#input\_create\_secret) | Whether to create a Secrets Manager secret for the master password | `bool` | `false` | no |
...
| <a name="input_db_cluster_parameter_group_parameters"></a> [db\_cluster\_parameter\_group\_parameters](#input\_db\_cluster\_parameter\_group\_parameters) | A list of DB cluster parameters to apply | <pre>list(object({<br/>    name  = string<br/>    value = string<br/>  }))</pre> | `[]` | no |
...
| <a name="input_event_categories"></a> [event\_categories](#input\_event\_categories) | List of event categories for a SourceType that you want to subscribe to | `list(string)` | `[]` | no |
| <a name="input_event_source_ids"></a> [event\_source\_ids](#input\_event\_source\_ids) | List of identifiers of the event sources for which events will be returned | `list(string)` | `[]` | no |
| <a name="input_event_source_type"></a> [event\_source\_type](#input\_event\_source\_type) | Type of source that will be generating the events. Valid values: db-instance, db-cluster, db-parameter-group, db-security-group, db-snapshot, db-cluster-snapshot | `string` | `"db-cluster"` | no |
| <a name="input_event_subscription_enabled"></a> [event\_subscription\_enabled](#input\_event\_subscription\_enabled) | Whether the subscription is enabled | `bool` | `true` | no |
| <a name="input_event_subscription_name"></a> [event\_subscription\_name](#input\_event\_subscription\_name) | Name of the event subscription | `string` | `null` | no |
...
| <a name="input_kms_key_description"></a> [kms\_key\_description](#input\_kms\_key\_description) | Description for the KMS key | `string` | `"DocumentDB cluster encryption key"` | no |
...
| <a name="input_ok_actions"></a> [ok\_actions](#input\_ok\_actions) | The list of actions to execute when this alarm transitions into an OK state | `list(string)` | `[]` | no |
...
| <a name="input_secret_description"></a> [secret\_description](#input\_secret\_description) | Description for the Secrets Manager secret | `string` | `"DocumentDB cluster master credentials"` | no |
| <a name="input_secret_name"></a> [secret\_name](#input\_secret\_name) | Name for the Secrets Manager secret | `string` | `null` | no |
| <a name="input_secret_recovery_window_in_days"></a> [secret\_recovery\_window\_in\_days](#input\_secret\_recovery\_window\_in\_days) | Number of days that AWS Secrets Manager waits before it can delete the secret | `number` | `7` | no |
...
| <a name="input_sns_topic_arn"></a> [sns\_topic\_arn](#input\_sns\_topic\_arn) | SNS topic ARN for event notifications | `string` | `null` | no |
